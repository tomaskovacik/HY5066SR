    ...
    while (btSerial.available()) {
      ...
      switch (state) {
        ...
        case PACKETSIZE:
          ...
          break;
        case DATA:
          if (packetbyte == data[0] - 1) //packetsize(data[0]) includes packet size plus one byte (checksum)
            state = CRC;
          else {
            data[++packetbyte] = n;
          }
        ...
        case END_23:
          if (n == 0x23) {
            //   Serial.println("end23");
            state = END_42;
          }
          break;
        case END_42:
          if (n == 0x42) {
            //Serial.println("end42");
            state = END_52;
          }
          break;
        case END_52:
          if (n == 0x52) {
            //Serial.println("end52");
            state = START_52;
